 ...
 * Handles video decoding, rendering, and playback control
 */
class VideoPlayer(videoPath: String, fullscreen: Boolean = true) {
  ...
  def initialize(): Try[Unit] = Try {
    ...
    decoder.initialize() match {
      case Success(_) =>
        ...
        logger.info(s"Video: ${width}x${height}, frame rate: ${frameRate.num()}/${frameRate.den()}")
        ...
        renderer.initialize(fullscreen) match {
          case Success(_) =>
            ...
            Success(())
          case Failure(e) =>
            throw new RuntimeException(s"Failed to initialize renderer: ${e.getMessage}", e)
        }
      case Failure(e) =>
        throw new RuntimeException(s"Failed to initialize decoder: ${e.getMessage}", e)
    }
  }
      ...
      if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        key match {
          case GLFW_KEY_SPACE =>
            ...
            logger.info(s"Playback: ${if (controller.isPlaybackPlaying) "PLAYING" else "PAUSED"}")
            
          case GLFW_KEY_LEFT =>
            val currentTime = controller.getCurrentTime
            val newTime = math.max(0, currentTime - 5.0) // Seek back 5 seconds
  ...
  def run(): Unit = {
    ...
    logger.info("Starting playback loop (paused). Controls: SPACE=play/pause, LEFT/RIGHT=seek, ESC/Q=exit")
        ...
        decoder.readFrame() match {
          case Some(frame) =>
            ...
            if (controller.getCurrentFrameIndex % 30 == 0) {
              logger.debug(s"Frame: ${controller.getCurrentFrameIndex}, Time: ${controller.getCurrentTime}s")
            }
